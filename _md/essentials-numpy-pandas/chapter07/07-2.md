...
이 라이브러리는 위니북스에서 바로 사용할 수 없으므로 코랩이나 주피터 노트북 등을 사용하여 실습해보시기 바랍니다.
...
plt.title('Sin') # 그래프의 제목을 설정합니다.
plt.xlabel('x') # x축의 레이블을 설정합니다.
plt.ylabel('sin(x)') # y축의 레이블을 설정합니다.
# plt.show() # 코랩 환경에서 그래프를 화면에 표시합니다.

display(plt.gcf(), target="py-internal-0-1")
```

위니북스에서는 `display()` 함수를 사용하여 그래프를 출력합니다. `display()` 함수는 그래프를 출력할 때 사용되며, `plt.gcf()` 함수는 현재 그래프를 가져옵니다. `target` 출력 위치 id를 지정합니다. 다른 환경에서 실습할 때에는 `display()` 함수 대신 `plt.show()` 함수를 사용하시기 바랍니다.
