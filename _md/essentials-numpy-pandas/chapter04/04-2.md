...
`Series(시리즈)`는 Pandas의 1차원 데이터 배열로, `인덱스와 값`으로 이루어진 데이터 구조입니다. 여기서 인덱스는 우리가 일반적으로 사용했던 숫자형 인덱스가 아니라, 사용자가 지정한 이름표를 의미합니다. 이를 통해 데이터를 더 쉽게 식별하고 관리할 수 있습니다. Series는 주로 데이터프레임의 열을 나타내는 데 사용됩니다.

# 2. Series 생성

Series를 생성하는 네 가지 방법에 대해 알아보겠습니다. 가장 먼저 모듈을 불러오는 것부터 시작합니다. Pandas 라이브러리는 `pd`라는 이름으로 불러옵니다. 앞서 배운 NumPy도 함께 사용할 것이므로 NumPy도 함께 불러옵니다.
...
`s.index`를 사용하면 해당 Series의 인덱스를 얻을 수 있습니다. `s.values`를 사용하면 Series에 저장된 실제 데이터 값들을 NumPy 배열 형태로 얻을 수 있습니다.
...
`s.iloc[]`를 사용하면 순서대로 데이터를 찾습니다. 예를 들어, `s.iloc[1]`이라고 하면 Series의 두 번째 데이터를 가져옵니다. `iloc`는 `integer location`의 약자입니다.

`s.loc[]`를 사용하면 인덱스 레이블을 이용해 데이터를 찾습니다. 예를 들어, `s.loc['c']`라고 하면 `c`라는 인덱스 레이블에 매핑된 데이터를 가져옵니다.
