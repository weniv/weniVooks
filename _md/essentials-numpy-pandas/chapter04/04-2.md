...
`Series(시리즈)`는 Pandas의 1차원 데이터 배열로, `인덱스와 값`으로 이루어진 데이터 구조입니다. 여기서 인덱스는 우리가 일반적으로 사용했던 숫자형 인덱스가 아니라, 사용자가 지정한 레이블이나 이름표를 의미합니다. 이를 통해 데이터를 더 쉽게 식별하고 관리할 수 있습니다. 

Series는 NumPy의 1차원 배열과 유사하지만, 인덱스를 사용하여 데이터에 더 쉽게 접근할 수 있습니다. Series는 리스트나 배열과 같은 데이터 구조를 사용하여 생성할 수 있으며, 딕셔너리와 같은 데이터 구조를 사용하여 생성할 수도 있습니다. Series는 데이터를 저장하고 관리하는 데 유용한 다양한 메서드와 속성을 제공합니다. 이러한 메서드와 속성을 활용하면 데이터를 더 쉽게 조작하고 분석할 수 있습니다.

# 2. Series 생성

Series를 생성하는 네 가지 방법에 대해 알아보겠습니다. 

첫 번째 방법은 리스트를 사용하여 Series를 생성하는 것입니다. 이 방법은 기본 인덱스, 즉 0부터 시작하는 정수를 가진 Series를 만들어냅니다.
...
세 번째 방법은 사용자가 직접 정의한 인덱스로 Series를 생성하는 것입니다. 이 방법은 리스트와 함께 사용자가 지정한 인덱스를 사용하여 Series를 만듭니다. 예를 들어, `pd.Series([1, 3, 5, 7, 9], index=['a', 'b', 'c', 'd', 'e'])`와 같이 작성하면 리스트를 인덱스 `['a', 'b', 'c', 'd', 'e']`와 함께 Series로 변환할 수 있습니다.
...
이렇게 네 가지 방법을 통해 다양한 형태의 데이터를 Series로 만들 수 있습니다. 각 방법은 상황에 따라 적절한 방법을 선택하면 됩니다.
...
**명시적 인덱스(Explicit Index)**

명시적 인덱스는 사용자가 직접 지정한 인덱스입니다. 즉, 인덱스를 임의로 설정할 수 있으며, 이를 통해 데이터를 쉽게 조회할 수 있습니다. 명시적 인덱스는 인덱스 레이블을 사용하여 데이터를 찾습니다.

**묵시적 인덱스(Implicit Index)**

묵시적 인덱스는 판다스가 자동으로 생성하는 인덱스입니다. 묵시적 인덱스는 0부터 시작하는 정수형 인덱스로, 데이터프레임과 시리즈를 생성할 때 index 속성을 설정하지 않으면 자동으로 생성됩니다. 묵시적 인덱스는 순서대로 데이터를 찾을 때 사용합니다.

## 3.2 요소 접근
![피그잼](/images/essentials-numpy-pandas/chapter04/loc_iloc.png )

Series에서 특정 데이터를 찾는 방법에는 두 가지가 있습니다. 첫 번째는 `s.iloc[]`를 사용하는 방법입니다. 이는 마치 배열에서 순서대로 데이터를 찾는 것과 비슷합니다. 예를 들어, `s.iloc[1]`이라고 하면 Series의 두 번째 데이터를 가져옵니다. 

두 번째 방법은 `s.loc[]`를 사용하는 것입니다. 이 방법은 우리가 데이터에 붙인 인덱스 레이블를 이용해 데이터를 찾습니다. 예를 들어, `s.loc['c']`라고 하면 `c`라는 인덱스 레이블에 매핑된 데이터를 가져옵니다.
...
Series에서 슬라이싱은 데이터의 특정 부분을 선택하여 추출하는 기능입니다. 슬라이싱은 NumPy 배열과 유사하게 동작하며, 인덱스를 사용하여 데이터를 선택합니다. 슬라이싱은 인덱스 레이블을 사용할 수도 있고, 숫자 인덱스를 사용할 수도 있습니다. 여기서 주의할 점은, 인덱스 레이블을 사용할 때는 마지막 인덱스도 포함된다는 점입니다.

```python-exec
import pandas as pd

s = pd.Series([1, 3, 5, 7, 9], index=['a', 'b', 'c', 'd', 'e'])
print("슬라이싱 (0부터 2까지):", s[0:2])
...
블리언 인덱싱(Boolean Indexing)은 Series에서 특정 조건을 만족하는 데이터만 선택하는 강력한 기능입니다. 앞서 학습한 NumPy의 불리언 인덱싱과 유사하게 작동합니다.
...
print("5보다 큰 값:", s[s > 5])
print("짝수인 값:", s[s % 2 == 0])
print("선택된 값:", s[[True, False, True, False, True]])
```

가장 마지막에 나온 것처럼 결국 불리언 인덱싱은 True와 False로 이루어진 리스트를 사용하여 데이터를 선택합니다. 이를 통해 사용자는 원하는 데이터만 선택하여 처리할 수 있습니다. 여기서 주의해야 할 점은 불리언 인덱싱의 괄호가 두 개라는 것입니다. 이는 불리언 인덱싱이 먼저 수행되어야 하기 때문입니다.
...
결측치 처리는 데이터 분석의 결과에 큰 영향을 미칠 수 있으므로 신중하게 접근해야 합니다. 어떤 방법을 선택할지는 데이터의 특성과 분석의 목적에 따라 달라집니다.
...
s_random = pd.Series(np.random.randn(5), 
                     index=['b', 'a', 'c', 'e', 'd'])
