...
---
"Pandas의 Series와 DataFrame은 데이터 분석과 조작에 있어 핵심적인 도구입니다. 이 연습 문제 세트는 Series와 DataFrame의 다양한 기능과 메서드를 깊이 있게 다룹니다. 기본적인 데이터 접근부터 복잡한 그룹화 연산, 결측치 처리, 데이터 병합 등 실제 데이터 분석 시나리오에서 자주 마주치는 상황들을 포함하고 있습니다. 이 문제들은 단순히 개념을 아는 것을 넘어, 실제로 코드를 작성하고 결과를 예측하는 능력을 테스트합니다. Series의 기본 조작부터 DataFrame의 고급 기능까지, 각 문제를 풀어나가면서 Pandas의 강력한 기능들을 어떻게 조합하고 활용할 수 있는지 배우게 될 것입니다.

![DALE](/images/essentials-numpy-pandas/chapter06/pandas_dataframe_qna.png )
...
## 문제 2
Series의 `apply()` 메서드를 사용할 때, 다음 중 가장 효율적인 방법은?
...
## 문제 5
Series의 `value_counts()` 메서드 사용 시, 상대 빈도를 계산하려면?
...
## 문제 6
다음 중 Series의 결측치를 가장 효과적으로 처리하는 방법은?
...
## 문제 7
MultiIndex Series에서 특정 레벨의 인덱스만 선택하는 올바른 방법은?
...
## 문제 9
Series의 `astype()` 메서드로 데이터 타입을 변환할 때, 'category' 타입으로 변환하면 어떤 이점이 있나요?
...
## 문제 11
DataFrame에서 특정 열을 삭제하려면 어떤 메서드를 사용해야 하는가?
...
## 문제 14
다음 중 DataFrame의 행을 합치는 방법으로 올바른 것은?
...
## 문제 17
다음 중 DataFrame의 특정 열을 기준으로 행의 순서를 변경하는 방법은?
...
## 문제 18
DataFrame에서 모든 결측치를 포함한 행을 삭제하려면?
...
## 문제 20
다음 중 DataFrame에서 열의 데이터 타입을 변경하는 메서드는?
...
## 문제 22
`fillna()` 메서드를 사용할 때, 다음 중 올바르지 않은 것은?
...
## 문제 24
`sort_values()` 메서드를 사용할 때, 다음 중 올바르지 않은 것은?
...
## 문제 27
DataFrame에서 특정 열을 기준으로 그룹화하고 여러 집계 함수를 적용하려면?
...
![DALE](/images/essentials-numpy-pandas/chapter06/pandas_answer.png )
