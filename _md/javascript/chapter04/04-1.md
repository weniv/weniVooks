...
console.log(numbers.join('-')); // "010-1111-2222"

const fruits = ['apple', 'banana', 'cherry'];
console.log(fruits.join()); // "apple,banana,cherry" - 기본 구분자인 쉼표(,)로 연결
```

`join()` 메서드를 사용할 때 주의할 점은 숫자를 문자열로 연결할 때 0으로 시작하는 숫자는 다른 진수로 해석될 수 있다는 것입니다. 예를 들어, 010은 8진수로 해석됩니다. 해당 예제는 위니북스에는 엄격모드와 같은 실행환경을 제공하고 있기 때문에 오류가 발생하니 콘솔에서 확인해주세요.

```javascript-exec
const arr = [010, 1111, 2222];
console.log(arr.join('-')); // "8-1111-2222" - 010은 8진수로 해석됨
...
이러한 점을 유의하면서 `join()` 메서드를 사용하면 배열을 쉽게 문자열로 변환할 수 있습니다.
