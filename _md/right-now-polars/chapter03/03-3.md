...
- 모든 작업이 표현식 내에서 수행됩니다.
- 가능한 한 모든 변환을 한 번의 `with_columns`에서 처리합니다.
- 메서드 체이닝을 통한 효율적인 연산 지원합니다.

## 6.1 시계열 데이터 생성

```python
# 날짜 범위 생성
dates = pl.date_range(start="2023-01-01", end="2023-01-05", interval="1d")
print(dates)
...
```
col("2023-01-01").date_range([col("2023-01-05")])
2020-03-14 15:00:00.alias("datetime")
```

## 6.2 문자열을 datetime으로 변환

- 형식 변환 시, `strptime`/`strftime` 사용합니다.
...
- `dt` 접근자 대신 표현식 내에서 직접 `dt` 메서드 사용합니다.
