...
`break`문은 `switch`문에서 중요한 역할을 합니다. 위의 코드처럼 `break`가 없다면, 조건이 참인 case 이후, 조건 평가 없이 모든 코드를 실행합니다. 이 현상을 **fall through**라고 합니다. 이를 방지하기 위해 각 `case` 뒤에 `break`를 추가해야 합니다. `break`를 추가하여 코드를 수정해보겠습니다.
