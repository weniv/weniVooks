...
이벤트는 사용자의 행동을 감지하여 특정 작업을 수행하는 기능입니다.
...
이벤트는 사용자의 행동을 감지하여 특정 작업을 수행하는 기능입니다. 자바스크립트에서는 `addEventListener()` 메서드를 사용하여 이벤트를 추가할 수 있습니다. 이 메서드는 두 개의 인수를 받습니다. 첫 번째 인수는 이벤트 타입(예: `click`, `mouseover`, `keydown` 등)이고, 두 번째 인수는 이벤트가 발생했을 때 실행될 함수(리스너 함수 또는 핸들러 함수)입니다.
