...
쉼표 연산자는 왼쪽 피연산자부터 차례대로 평가하고, 마지막 피연산자의 평가 결과를 반환합니다.
...
여러 변수를 한 번에 초기화는 등 여러 표현식을 하나로 결합하는데 사용됩니다.

# 6. 그룹 연산

그룹 연산자는 `()`로 표현하며, 연산자 우선순위를 변경하거나 표현식을 그룹화할 때 사용합니다.
...
지금까지 배운 다양한 연산자들에는 우선순위가 있습니다. 연산자 우선순위로 표현식이 평가되는 순서를 예측할 수 있습니다.
| 우선순위 | 연산자 유형 |
|---------|------------|
| 1 | 그룹 (`()`) |
| 2 | 옵셔널 체이닝 (`?.`), 함수 호출 (`()`), 배열 요소 접근 (`[]`), 객체 속성 접근 (`.`) |
|3|증감연산자 (`++`, `--`)|
|4|부정 (`!`), 단항 덧셈 (`+`), 단항 뺄셈 (`-`), 타입 변환 (`typeof`), `delete`|
|5| 지수 연산자 (`**`)|
|6|곱셈 (`*`), 나눗셈 (`/`), 나머지 (`%`)|
|7|덧셈 (`+`), 뺄셈 (`-`)|
|8|비교 (`<`, `>`, `<=`, `>=`), `instanceof`, `in`|
...
::a[연산자 우선순위 | MDN]{class='btn-link' href='https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Operators/Operator_precedence#%ED%91%9C' target='\_blank'}
