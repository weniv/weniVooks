...
키워드를 생략하고 변수의 할당문을 작성하면 해당 변수는 전역 객체의 속성으로 등록됩니다. 즉, `window` 객체의 속성으로 등록됩니다. 이는 `var` 키워드로 변수를 선언한 것과 동일하게 동작합니다. `var`는 `let`과 `const`가 도입되기 전에 변수를 선언하는 키워드입니다. 이처럼 `var`는 변수를 안전하게 사용하는데 문제가 될 수 있기 때문에 변수를 선언할 때는 `let`과 `const`를 사용하는 것을 권장합니다.
:::
