...
1. React가 리스트 아이템의 변경, 추가, 제거를 효율적으로 처리할 수 있게 합니다.
2. 가능한 한 고유하고 안정적인 ID를 key로 사용해야 합니다.
3. 인덱스를 key로 사용하는 것은 리스트 순서가 바뀔 수 있는 경우 문제를 일으킬 수 있으므로 피해야 합니다.

:::

이 코드는 과일 목록을 동적으로 생성하는 방법을 보여줍니다. 먼저 각 과일에 고유한 id를 부여하고, 이를 key prop으로 사용합니다. 그 다음, JSX 안에서 map 함수를 사용하여 이 배열의 각 항목을 리스트 아이템(<li>)으로 변환합니다. 각 리스트 아이템에는 key 속성이 필요한데, 여기서는 간단히 배열의 인덱스를 사용했습니다. 또한 각 과일의 색상을 스타일로 적용하여 더 풍부한 UI를 생성합니다.
...
function SubmitButton({ onClick, className }) {
  return (
    <Button onClick={onClick} className={`submit-button ${className}`}>
      제출
    </Button>
  );
}
...
function App() {
  return (
    <div>
      ...
      <SubmitButton onClick={() => console.log('제출')} />
    </div>
  );
}
```

이 예제에서는 기본 Button 컴포넌트를 특수화하여 DeleteButton과 SubmitButton을 만듭니다. 이 방식은 공통 기능을 유지하면서 특정 용도에 맞는 변형을 만들 수 있게 해줍니다.
...
이 예제는 UserProfile 컴포넌트를 여러 작은 컴포넌트(UserAvatar, UserInfo, UserStats)로 나누는 방법을 보여줍니다. 먼저 Avatar 컴포넌트는 사용자의 프로필 이미지를 표시합니다. UserInfo 컴포넌트는 사용자의 이름과 이메일 정보를 보여줍니다. 마지막으로 UserProfile 컴포넌트는 이 작은 컴포넌트들을 조합하여 전체 사용자 프로필을 구성합니다. 이렇게 컴포넌트를 작은 단위로 나누면 각 부분을 독립적으로 수정하고 재사용하기 쉬워집니다. 또한 코드의 가독성과 유지보수성이 향상됩니다.
...
Render Props 패턴은 컴포넌트의 렌더링 로직을 외부에서 제어할 수 있게 해줍니다. 이 예제에서 MouseTracker 컴포넌트는 마우스 위치를 추적하고, 이 정보를 render prop을 통해 전달받은 함수에 제공합니다.
...
이 예제는 주요 생명주기 메서드의 실제 사용을 보여줍니다. componentDidMount에서 초기 데이터를 가져오고, componentDidUpdate에서 prop 변경에 반응하며, componentWillUnmount에서 정리 작업을 수행합니다.
...
function Counter() {
  ...
  // 부수 효과 처리 (이전의 componentDidMount, componentDidUpdate, componentWillUnmount 역할)
  useEffect(() => {
...
:::div{.callout}
*클래스 컴포넌트를 함수형 컴포넌트로 변환*할 때, 주의 사항은 다음과 같습니다.
...
5. this 키워드 사용을 피합니다.
6. 상태 업데이트 로직이 이전 상태에 의존할 경우, 함수형 업데이트를 사용합니다.
