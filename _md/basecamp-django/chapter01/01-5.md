...
이 섹션을 시작하기 전에, 1.4장에서 설명한 Django 설치 및 프로젝트 생성 과정을 완료해야 합니다. 이 과정을 통해 기본적인 Django 프로젝트 구조가 생성되었다고 가정하고 진행합니다.

# 1. 앱 생성

Django에서는 기능별로 앱을 만들어 관리합니다. 가상환경이 활성화되어 있는지 확인해주세요. VSC 터미널(터미널 > 새 터미널)에 다음 명령어를 입력해 'main' 앱을 생성합니다.
...
- 첫 번째 파라미터: 전화번호(URL) (예: **"a/"**)
- 두 번째 파라미터: 담당 직원(함수) (예: **a**)
...
3. 메인 페이지가 표시되는 것을 확인한 후, 다른 URL들(예: '/a/', '/b/', '/c/', '/hojun/', '/orm/')도 차례로 방문해 봅니다.
