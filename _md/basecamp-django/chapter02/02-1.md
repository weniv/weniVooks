...
date: 2024-07-09
---
URL 처리는 웹사이트에서 사용자가 특정 주소를 입력했을 때, 그에 맞는 페이지를 보여주는 방법입니다. Django에서는 이을 통해 깔끔하고 이해하기 쉬운 웹 주소를 만들 수 있습니다.
...
그럼 실습을 통해 이 개념들을 직접 적용해보면서 Django의 강력한 기능들을 경험해봅시다.

# 1. 개발 환경 설정

1. Visual Studio Code를 실행하고 작업할 폴더를 엽니다. `File > Open Folder`에서 Django 프로젝트 폴더를 선택하면 됩니다.
...
:::div{.callout}
위의 '개발 환경 설정'과 'Django 프로젝트 생성' 과정은 모든 Django 프로젝트의 **기초** 입니다. 이후 실습에서는 이 과정을 반복하지 않으므로, 새로운 프로젝트를 시작할 때마다 이 단계들을 **먼저** 수행해주세요. 환경 설정이 완료된 후 실습을 진행하시면 됩니다. 더 자세한 내용은 1장의 1.3 Django 설치 및 프로젝트 생성을 참고해주세요.
...
1. 위의 url 구조에 맞춰 `tutorialdjango > urls.py` 파일을 다음과 같이 수정합니다.
   ...
   ```
`path()`는 Django에서 웹사이트의 주소와 그 주소에 해당하는 작업을 연결해주는 역할을 합니다. `path`에 대한 자세한 이야기는 02-3에서 다루도록 하겠습니다.

# 5. 뷰 함수 구현
설계한 URL에 접속했을 때 실제 보게될 화면을 `main > view.py`에서 구현할 수 있습니다.
이 부분에서는 `HttpResponse`을 사용한 방식과, `render`을 사용한 방식을 보여드리고자 합니다.
...
단순하고 직접적인 응답을 생성할 때 유용하지만, 복잡한 HTML이나 대량의 데이터를 다룰 때는 비효율적일 수 있습니다. 이런 이유로 대부분의 실제 웹 어플리케이션에서는 `render`을 주로 사용합니다.
...
이 실습에서는 render 방식을 사용해주시면 됩니다.

# 6. 템플릿 생성

1. `main` 앱 폴더 안에 `templates` 폴더를 **생성**합니다. 
`templates` 폴더 안에 다시 한 번 `main`폴더를 만들어 주세요.
