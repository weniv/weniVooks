...
title: URL처리와 템플릿 태그 실습 1
date: 2024-07-09
---
# 1. 개발 환경 설정

1. Visual Studio Code를 실행하고 작업할 폴더를 엽니다.
   - File > Open Folder에서 Django 프로젝트 폴더를 선택합니다.
...
:::div{.callout}
위의 '개발 환경 설정'과 'Django 프로젝트 생성' 과정은 모든 Django 프로젝트의 **기본 시작점**입니다. 이후 실습에서는 이 과정을 반복하지 않으므로, 새로운 프로젝트를 시작할 때마다 이 단계들을 **먼저** 수행해주세요. 환경 설정이 완료된 후 각 실습의 고유한 내용을 진행하면 됩니다.
:::
...
# 5. 뷰 함수 구현
- 설계한 URL에 접속했을 때 실제 보게될 텍스트는 `main > view.py`에서 구현 할 수 있습니다. 
...
1. `main` 앱 폴더 안에 `templates/main` 폴더를 **생성**합니다.

2. `templates/main` 폴더 안에 다음 HTML 파일들을 **생성**합니다.
   - `blog_list.html`
   - `blog_details.html`
   - `accounts_details.html`

3. 각 HTML 파일에 아래 내용을 추가합니다.
