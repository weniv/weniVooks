...
`사용자(들)`을 클릭해보면, 유저가 한명 더 생긴것을 확인할 수 있습니다.

게시물을 3개 더 작성해 줍니다. 이때 Author은 이미지와 같이 새로 만든 유저로 설정해 주세요.
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-10.png"}

게시물이 6개가 되었습니다.
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-11.png"}

이제 유저를 삭제해보겠습니다. `사용자(들)`에서 삭제할 유저를 선택하고, 액션에서 삭제 선택, 실행을 누르면 유저가 삭제됩니다.
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-12.png"}
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-13.png"}

`leejojun2`으로 만든 게시물이 삭제된 것을 확인 할 수 있습니다.
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-14.png"}

```python
author = models.ForeignKey(User, on_delete=models.SET_NULL)
```
위 코드에서 `models.CASCADE`대신 `models.SET_NULL`을 넣으면 삭제가 아니라 빈칸이 됩니다.
`on_delete=models.SET_NULL`을 사용하고 싶으면, `null=True` 같이 이 필드에 null값이 들어 갈 수 있게 설정해야합니다.
::img{width="800" alt="" src="/images/basecamp-django/chapter03/04-15.png"}
## 10.2 blog_list.html 수정
...
`태그`는 여러 게시물에서, 여러 태그를 가질 수 있습니다. 그러니 게시물과 태그는 `다대다(N : M)`관계를 이룹니다.
`댓글`은 하나의 게시물에서 여러 댓글을 가질 수 있습니다. 그러니 게시물과 댓글은 `일대다(1 : N)`관계를 가집니다.
앞서 실습했던 것과 같이 일대다 관계에는 `ForeingnKey`를 다대다 관계에는 `ManyToManyField`를 사용합니다.     

우선 models.py를 수정해줍니다.
```python
#blog > models.py
from django.db import models
from django.contrib.auth.models import User
