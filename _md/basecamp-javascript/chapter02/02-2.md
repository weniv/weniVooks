...
숫자형은 숫자를 처리하기 위해 사용되는 자바스크립트의 자료형입니다. 이 장에서는 숫자형의 다양한 특징과 기능을 살펴보겠습니다.
...
// 숫자형 데이터에 알파벳 e를 붙이면 0을 손쉽게 추가할 수 있습니다.
...
:::div{.callout}
`3.toString()`은 왜 에러가 발생할까요? 자바스크립트 엔진은 숫자 뒤의 점을 소수점으로 인식하기 때문입니다. 따라서 `(3).toString()`처럼 소괄호를 사용해야 합니다.
:::
...
console.log(`Math.round(4.7) : ${Math.round(4.7)}`); // 5 (반올림, 4.7을 반올림하여 5로 반환)
console.log(`Math.round(4.4) : ${Math.round(4.4)}`); // 4 (반올림, 4.4를 반올림하여 4로 반환)

console.log(`Math.pow(2, 8) : ${Math.pow(2, 8)}`); // 256 (2의 8승)

console.log(`Math.sqrt(64) : ${Math.sqrt(64)}`); // 8 (64의 제곱근)

console.log(`Math.abs(-5) : ${Math.abs(-5)}`); // 5 (절대값, -5의 절대값은 5)

console.log(`Math.random() : ${Math.random()}`); // 0과 1 사이의 랜덤 숫자 반환

console.log(`Math.max(10, 20, 30, 40, 50) : ${Math.max(10, 20, 30, 40, 50)}`); // 50 (최대값 반환)
console.log(`Math.min(10, 20, 30, 40, 50) : ${Math.min(10, 20, 30, 40, 50)}`); // 10 (최소값 반환)
```
