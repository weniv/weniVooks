...
코드에 대한 주요 개념을 먼저 설명하고, 코드 구현은 아래 섹션에서 진행하겠습니다. 헷갈리고 생소한 개념이 많을 수 있으니 천천히, 반복해서 읽어보세요.
...
## 3.3 주요 함수
- `verify_password`: 평문 비밀번호와 해시된 비밀번호를 비교합니다.
- `get_password_hash`: 비밀번호를 해시합니다.
- `get_user`: 사용자 이름으로 사용자 정보를 조회합니다.
- `authenticate_user`: 사용자 인증을 수행합니다.
- `create_access_token`: JWT 액세스 토큰을 생성합니다.
- `get_current_user`: 현재 인증된 사용자 정보를 반환합니다.
