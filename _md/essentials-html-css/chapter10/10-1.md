...
전체 선택자(universal selector)는 HTML 문서 내의 모든 요소를 선택합니다. 즉, html 태그를 포함한 모든 태그에 스타일을 적용할 수 있습니다. 전체 선택자는 `*` (별표, 애스터리스크)를 사용하여 지정할 수 있습니다. 이 선택자는 보통 reset.css 파일에서 많이 사용됩니다. reset.css는 이러한 전체 선택자를 통해 각기 다른 브라우저마의 기본 스타일을 초기화하여 일관된 디자인을 가능하게 합니다.
