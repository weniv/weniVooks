...
RouterProvider 컴포넌트는 최상위 컴포넌트로 전체 애플리케이션을 감싸고 있어, 앱 전체에 라우팅 기능을 제공합니다. 브라우저의 URL이 변경되면, RouterProvider는 현재 URL을 createBrowserRouter로 정의한 라우트 설정과 비교합니다. 일치하는 경로를 찾으면, 해당 경로에 정의된 element(컴포넌트)를 렌더링합니다. 위 예시에서는 `localhost:5173/` 와 `localhost:5173/weniVooks` 로 접속하면 각각 "Hello World!"와 "Welcome to weniVooks!"라는 문구가 보일 것입니다.
...
`/user/:id`에 접속하면 Root 컴포넌트 내의 `<Outlet />` 위치에 UserProfile 컴포넌트가 렌더링됩니다. `useParams` 훅을 사용해 파라미터를 추출하고 파라미터 값을 페이지에 표시합니다. 따라서 `/user/user1`, `/user/user2` 등의 URL로 접속하면 해당 id 값을 포함한 페이지가 표시됩니다.
