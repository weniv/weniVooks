...
React를 사용하여 이미지 검색 애플리케이션을 만들어보겠습니다. 이 프로젝트는 `검색바`, `이미지 리스트`, `개별 이미지 컴포넌트`로 구성됩니다.
...
src 폴더 내 기존 파일을 삭제하신 후, 다음 파일들을 생성해 주세요.
...
App 컴포넌트는 애플리케이션의 주요 상태를 관리하고, 검색 기능을 구현합니다.
...
App.js는 애플리케이션의 중심 컴포넌트입니다. 여기서 이미지 검색 기능을 구현하고, 검색 결과와 에러 상태를 관리합니다. SearchBar와 ListImg 컴포넌트를 사용해 UI를 구성합니다.
...
개별 이미지 아이템을 표시합니다. React.memo를 사용하여 성능을 최적화합니다.
