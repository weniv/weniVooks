...
function Button({ text, color, onClick }) {
  return (
    <button 
      style={{ backgroundColor: color, color: 'white', padding: '10px', border: 'none', borderRadius: '5px' }}
      onClick={onClick}
    >
      {text}
    </button>
...
function Article({ title }) {
  return <article><h4>{title}</h4></article>;
}

// Footer 컴포넌트
function Footer() {
  return <footer><p>© 2024 나의 웹사이트. All rights reserved.</p></footer>;
}
...
function Welcome({ name, age }) {
    return <h1>안녕하세요, {name}님! 당신은 {age}살입니다.</h1>;
}
...
2. CSS 프레임워크 Style 적용 `<Tailwind CSS>`

3. 모듈 CSS 적용

4. StyledComponent 적용
