...
위 예시 코드에서, 부모 컴포넌트는 두 개의 자식 컴포넌트를 렌더링하여 각각에 다른 name과 age props를 전달합니다. 이를 통해 자식 컴포넌트는 전달받은 props를 사용하여 개인화된 정보를 표시합니다. 결과적으로 화면에는 다음과 같은 정보가 표시될 것입니다.
...
위 예시를 통해 다음과 같은 정보들을 파악할 수 있습니다.

1. useState 훅

`useState`는 React의 훅 중 하나로, 함수 컴포넌트에서 state를 사용할 수 있게 해줍니다. 이러한 useState(0)는 초기값이 0인 state를 생성합니다. 즉, useState 훅을 사용하여 state를 선언하고 초기화하는 것입니다.
`count`는 현재 state 값이고, `setCount`는 state를 업데이트하는 함수입니다. 즉, state는 직접 수정하지 않고, 항상 setState 함수(여기서는 setCount)를 통해 업데이트하는 것입니다.

2. state 사용

JSX 내에서 `{count}`로 현재 state 값을 표시합니다. 이러한 state는 컴포넌트의 변경 가능한 데이터를 저장합니다.

3. state 업데이트

`increment` 함수에서 `setCount(count + 1)`를 호출하여 state를 업데이트합니다. 버튼을 클릭한다면, 이 함수가 실행되어 count가 1씩 증가합니다.

4. 리렌더링

`setCount`가 호출되면 React는 컴포넌트를 다시 렌더링하여 새로운 state 값을 반영합니다. state가 변경되면 컴포넌트가 자동으로 다시 렌더링되는 과정입니다.
