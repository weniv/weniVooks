...
부모 컴포넌트가 리렌더링되면 기본적으로 자식 컴포넌트도 리렌더링됩니다. 그러나 자식 컴포넌트의 props가 변경되지 않았다면, 이러한 리렌더링은 불필요할 수 있습니다. 특히 자식 컴포넌트가 복잡하거나 큰 경우, 이는 성능 저하로 이어질 수 있습니다. 이러한 불필요한 리렌더링을 방지하기 위해 useMemo와 React.memo를 함께 사용할 수 있습니다.
...
const ChildComponent = ({ title, value, renderCount }) => {
...
const NonMemoizedChild = ({ data, renderCount }) => (
    <ChildComponent
        title="최적화되지 않은 컴포넌트"
        value={data.value}
...
const MemoizedChild = React.memo(({ data, renderCount }) => (
    <ChildComponent
        title="최적화된 컴포넌트"
        value={data.value}
        renderCount={renderCount}
...
const ParentComponent = () => {
    ...
    const [nonMemoRenderCount, setNonMemoRenderCount] = useState({ current: 0 });
    const [memoRenderCount, setMemoRenderCount] = useState({ current: 0 });

    const memoizedData = useMemo(() => ({ value: data }), [data]);
    const unmemoizedData = { value: data };
    ...
    return (
        <div className="parent-component">
            ...
            <div className="button-container">
                <button onClick={() => {
                    setCount(c => c + 1);
                    setNonMemoRenderCount(prev => ({ current: prev.current + 1 }));
                }}>
                    Count 증가: {count}
                </button>
                <button onClick={() => {
                    setData(d => d + 1);
                    setNonMemoRenderCount(prev => ({ current: prev.current + 1 }));
                    setMemoRenderCount(prev => ({ current: prev.current + 1 }));
                }}>
                    Data 증가: {data}
                </button>
            </div>
            <div className="children-container">
                <NonMemoizedChild data={unmemoizedData} renderCount={nonMemoRenderCount} />
                <MemoizedChild data={memoizedData} renderCount={memoRenderCount} />
            </div>
        </div>
    );
};
...
부모 컴포넌트인 ParentComponent는 두 가지 상태, `count`와 `data`를 관리합니다. 이 컴포넌트는 두 개의 버튼을 제공합니다. 하나는 count를, 다른 하나는 data를 증가시킵니다. 또한, 부모 컴포넌트는 각 자식 컴포넌트의 렌더링 횟수를 추적합니다.

자식 컴포넌트는 두 가지 유형이 있습니다. 하나는 최적화되지 않은 NonMemoizedChild이고, 다른 하나는 React.memo를 사용하여 최적화된 MemoizedChild입니다. 두 컴포넌트는 동일한 ChildComponent를 사용하지만, 최적화 여부에 따라 다르게 동작합니다.

부모 컴포넌트는 자식 컴포넌트들에게 데이터를 전달할 때 두 가지 방식을 사용합니다. NonMemoizedChild에게는 매 렌더링마다 새로 생성되는 객체인 unmemoizedData를 전달합니다. MemoizedChild에게는 useMemo 훅을 사용하여 생성한 memoizedData를 전달합니다. memoizedData는 data 상태가 변경될 때만 새로 생성됩니다.
`count`를 증가시키는 버튼을 클릭하면, 부모 컴포넌트와 NonMemoizedChild는 리렌더링됩니다. 그러나 MemoizedChild는 받는 데이터(memoizedData)가 변경되지 않았기 때문에 리렌더링되지 않습니다.

`data`를 증가시키는 버튼을 클릭하면, 모든 컴포넌트가 리렌더링됩니다. 이는 data의 변경이 두 자식 컴포넌트 모두에게 전달되는 데이터에 영향을 미치기 때문입니다.

각 컴포넌트의 렌더링 횟수를 직접 화면에서 확인할 수 있어, 최적화의 효과를 쉽게 이해할 수 있습니다. NonMemoizedChild는 부모 컴포넌트가 리렌더링될 때마다 함께 리렌더링되지만, MemoizedChild는 실제로 사용하는 데이터가 변경될 때만 리렌더링됩니다.
...
::a[React 공식 문서 - memo]{class='btn-link' href="https://react.dev/reference/react/memo" target="\_blank"}
