...
만약 numpy나 pandas에 대한 상세한 내용을 원하신다면, [NumPy Pandas essentials](https://www.books.weniv.co.kr/essentials-numpy-pandas)를 참고하시기 바랍니다. 이 수업은 모듈을 상세하게 살펴보기 보다는 이것을 무엇을 할 수 있는지 보여주는 것에 초점을 맞추고 있습니다.
...
다만 위니북스에서는 위의 코드를 실행시킬 수 없기 때문에 아래와 같은 코드를 사용해야 합니다. 이 이후의 코드는 대부분 유사합니다.
...
타입을 확인해보니 행과 열로 이뤄진 것은 데이터프레임이며, 생존자를 확인해보니 시리즈로 이뤄진 것을 확인할 수 있습니다.
...
이 코드를 실행하면 우리는 데이터셋의 크기, 첫 몇 행의 내용, 수치형 데이터의 기본 통계 정보, 그리고 각 열의 데이터 타입과 비어있지 않은 값의 개수를 볼 수 있습니다.

데이터 크기를 보면 이 데이터셋에는 891개의 행(승객)과 12개의 열(특성)이 있음을 알 수 있습니다. 이는 타이타닉호에 탑승했던 승객들의 표본 데이터라고 볼 수 있습니다.

데이터의 상위 5행을 보면 각 열이 어떤 정보를 담고 있는지 대략적으로 파악할 수 있습니다. 'PassengerId'는 각 승객의 고유 식별자, 'Survived'는 생존 여부(0: 사망, 1: 생존), 'Pclass'는 객실 등급, 'Name'은 승객의 이름, 'Sex'는 성별, 'Age'는 나이, 'SibSp'는 함께 탑승한 형제자매와 배우자의 수, 'Parch'는 함께 탑승한 부모와 자녀의 수, 'Ticket'은 티켓 번호, 'Fare'는 요금, 'Cabin'은 객실 번호, 'Embarked'는 승선 항구를 나타냅니다.

기본 통계 정보를 보면 각 수치형 특성의 평균, 표준편차, 최솟값, 최댓값 등을 확인할 수 있습니다. 예를 들어, 'Age' 열의 평균값은 약 29.7세이고, 최소 0.42세에서 최대 80세까지의 분포를 보입니다.
...
데이터 전처리는 분석의 정확성과 효율성을 높이기 위한 중요한 단계입니다. 이 과정에서는 결측치 처리, 불필요한 특성 제거, 범주형 데이터 인코딩 등을 수행합니다. 전처리 과정을 통해 데이터의 품질을 향상시키고, 후속 분석 단계에서 보다 신뢰할 수 있는 결과를 얻을 수 있습니다.

먼저, 불필요한 열을 제거하고 결측치를 처리해보겠습니다.
...
이 코드에서는 여러 가지 전처리 작업을 수행했습니다. 

첫째, 'PassengerId', 'Name', 'Ticket' 열을 제거했습니다. 이 특성들은 승객의 생존 여부와 직접적인 관련이 없다고 판단되어 제거했습니다. 'Name'의 경우 일부 유용한 정보(예: 직위)를 포함할 수 있지만, 이 분석에서는 단순화를 위해 제외했습니다.

둘째, 'Age' 열의 결측치를 중앙값(median)으로 대체했습니다. 중앙값을 사용한 이유는 평균값보다 극단값의 영향을 덜 받기 때문입니다. 나이 데이터의 경우 극단값(매우 어리거나 매우 나이 든 승객)이 있을 수 있어 중앙값이 더 적절한 대체값이 될 수 있습니다. 다만, 남자와 여자의 나이 평균이 다르기 때문에 성별에 따라 중앙값을 대체하는 것이 더 좋을 수 있습니다. 이러한 데이터 분석은 데이터의 특성에 따라 다르므로, 신중한 판단이 필요합니다. 머신러닝 결과로 성능이 좋지 않다면 이러한 전처리 방법을 바꿔보는 것도 좋습니다.

셋째, 'Embarked' 열의 결측치를 최빈값(mode)으로 대체했습니다. 승선 항구는 범주형 데이터이므로, 가장 많이 등장하는 값으로 대체하는 것이 합리적입니다.

넷째, 'Cabin' 열의 경우 결측치가 매우 많아 단순히 객실 정보의 유무로 변환했습니다. 이는 객실 번호 자체보다는 객실 배정 여부가 생존과 관련이 있을 수 있다는 가정 하에 수행한 전처리입니다.
