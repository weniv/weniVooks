...
# 복합 조건 (AND)
print("\n30세 이상이고 Paris에 사는 사람:")
print(df[(df['age'] >= 30) & (df['city'] == 'Paris')])

# 복합 조건 (OR)
print("\nParis나 London에 사는 사람:")
...
print("\nParis에 살고 30세 이상인 사람:")
print(df.query('city == "Paris" and age >= 30'))
```
...
# 복잡한 조건 조합
print("\n(IT 부서이면서 급여 6만 이상) 또는 (HR 부서이면서 경력 5년 이상)인 직원:")
...
# 여러 컬럼에 대한 집계
print("\n여러 컬럼 집계:")
print(df[['age', 'salary']].agg(['mean', 'median', 'std']))
```
...
merge 함수를 사용하여 데이터를 병합할 수 있습니다. 보통 데이터를 병합하는 `JOIN`은 아래와 같이 4가지 종류가 있습니다.
