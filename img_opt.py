...
class ImageOptimizer:
            ...
            if dir_path.exists():
                ...
                for file_path in dir_path.rglob('*'):
                    if (file_path.is_file() and 
                        ...
                        not file_path.name.startswith(self.config['backup_prefix'])):  # 백업 파일 제외
    ...
    def optimize_image(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if new_size > original_size:
                self.logger.warning(f"경고 {file_path.name}: 최적화 후 크기가 증가함. 원본으로 복원합니다.")
                # 백업에서 원본 복원
                if backup_path and backup_path.exists():
                    ...
                    self.logger.error(f"오류 {file_path.name}: 백업이 없어 복원할 수 없습니다.")
                    self.stats['errors'] += 1
                    return False
            ...
            reduction_text = f"{size_reduction:.1f}% 감소" if size_reduction >= 0 else f"{abs(size_reduction):.1f}% 증가"
            ...
            self.logger.error(f"오류 {file_path.name} 처리 실패: {str(e)}")
            self.stats['errors'] += 1
            return False
    ...
    def print_summary(self) -> None:
        ...
        reduction_percentage = (total_reduction / self.stats['total_size_before'] * 100) if self.stats['total_size_before'] > 0 else 0
        ...
        print(f"용량 절약: {self.format_size(total_reduction)} ({reduction_percentage:.1f}%)")
        print(f"전체 용량: {self.format_size(self.stats['total_size_before'])} -> {self.format_size(self.stats['total_size_after'])}")
        print("="*60)
